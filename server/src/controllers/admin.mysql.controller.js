...
async function ensureExamCentreByName(name) {
  ...
  let doc = await ExamCentreModel.findOne({ name: centreName }).lean();

  if (!doc) {
    const newCentre = new ExamCentreModel({
      name: centreName,
      status: 'active',
      created_at: new Date(),
      updated_at: new Date(),
  ...
  if (doc.id === undefined || doc.id === null) {
    const maxRow = await ExamCentreModel.findOne({ id: { $ne: null } }).sort({ id: -1 }).select({ id: 1 }).lean();
    const nextId = await getNextNumericId('exam_centres', Number(maxRow?.id || 0));
    await ExamCentreModel.updateOne(
...
async function ensureExamCentreYearActive(centreId, year) {
  ...
  const existing = await ExamCentreYearModel.findOne({ centre_id: cid, year: yr }).lean();
  if (!existing) {
    const doc = new ExamCentreYearModel({
      centre_id: cid,
      year: yr,
      status: 'active',
      created_at: new Date(),
  ...
  if (existing.id === undefined || existing.id === null) {
    const maxRow = await ExamCentreYearModel.findOne({ id: { $ne: null } }).sort({ id: -1 }).select({ id: 1 }).lean();
    const nextId = await getNextNumericId('exam_centre_years', Number(maxRow?.id || 0));
    await ExamCentreYearModel.updateOne(
...
export async function createExamCentre(req, res, next) {
  try {
    ...
    const ExamCentreModel = await getExamCentreModel();
    const centre = new ExamCentreModel({ name, status, created_at: new Date(), updated_at: new Date() });
    await centre.save();
    const doc = await ExamCentreModel.findOne({ id: centre.id }, { _id: 0, __v: 0 }).lean();
...
export async function listExamCentres(req, res, next) {
  try {
    const ExamCentreModel = await getExamCentreModel();
    const centres = await ExamCentreModel.find({}, { _id: 0, __v: 0 }).sort({ name: 1, id: 1 }).lean();
...
export async function updateExamCentre(req, res, next) {
  try {
    ...
    const ExamCentreModel = await getExamCentreModel();
    await ExamCentreModel.updateOne({ id: Number(id) }, { $set: updates });
    const doc = await ExamCentreModel.findOne({ id: Number(id) }, { _id: 0, __v: 0 }).lean();
...
export async function deleteExamCentre(req, res, next) {
  try {
    ...
    const ExamCentreModel = await getExamCentreModel();
    const existing = await ExamCentreModel.findOne({ id: Number(id) }, { id: 1, _id: 0 }).lean();
    if (!existing) return res.status(404).json({ message: 'Centre not found' });

    const ExamCentreYearModel = await getExamCentreYearModel();
    const PyqModel = await getPyqModel();
    await ExamCentreYearModel.deleteMany({ centre_id: Number(id) });
    await PyqModel.updateMany({ centre_id: Number(id) }, { $set: { centre_id: null } });
    await ExamCentreModel.deleteOne({ id: Number(id) });
...
export async function createExamCentreYear(req, res, next) {
  try {
    ...
    const ExamCentreModel = await getExamCentreModel();
    const ExamCentreYearModel = await getExamCentreYearModel();
    const centre = await ExamCentreModel.findOne({ id: Number(centreId) }, { id: 1, _id: 0 }).lean();
    ...
    await doc.save();
    const saved = await ExamCentreYearModel.findOne({ id: doc.id }, { _id: 0, __v: 0 }).lean();
...
export async function listExamCentreYears(req, res, next) {
  try {
    const { centreId } = req.params;
    const ExamCentreYearModel = await getExamCentreYearModel();
    const years = await ExamCentreYearModel.find({ centre_id: Number(centreId) }, { _id: 0, __v: 0 })
      .sort({ year: -1, id: -1 })
      .lean();
...
export async function updateExamCentreYear(req, res, next) {
  try {
    ...
    updates.updated_at = new Date();
    const ExamCentreYearModel = await getExamCentreYearModel();
    await ExamCentreYearModel.updateOne({ id: Number(id) }, { $set: updates });
    const doc = await ExamCentreYearModel.findOne({ id: Number(id) }, { _id: 0, __v: 0 }).lean();
...
export async function deleteExamCentreYear(req, res, next) {
  try {
    ...
    const ExamCentreYearModel = await getExamCentreYearModel();
    const PyqModel = await getPyqModel();
    const y = await ExamCentreYearModel.findOne({ id: Number(id) }, { id: 1, centre_id: 1, year: 1, _id: 0 }).lean();
    if (!y) return res.status(404).json({ message: 'Year not found' });

    const cnt = await PyqModel.countDocuments({ centre_id: Number(y.centre_id), year: String(y.year) });
    if (Number(cnt || 0) > 0) {
      return res.status(400).json({ message: 'Cannot delete year with existing PYQs' });
    }

    await ExamCentreYearModel.deleteOne({ id: Number(id) });
    return res.json({ ok: true });
  } catch (err) {
    return next(err);
  }
...
export async function manageStudentSubscription(req, res, next) {
  try {
    ...
    if (action === 'subscribe') {
      ...
      // Clear any prior admin revocation so auto-recovery doesn't get confused
      acc.admin_revoked_at = null;
      ...
      // ── Stamp revocation time ──────────────────────────────────────────
      // The student payment auto-recovery will only re-grant access if the
      // student's payment.paid_at is AFTER this timestamp, preventing the
      // auto-recovery from silently undoing admin revocations.
      acc.admin_revoked_at = new Date();
    }

    acc.updated_at = new Date();
    await acc.save();

    // Bust the student's access status cache so the change is immediate
...
export async function deleteStudent(req, res, next) {
  try {
    ...
    // Delete results from both main DB and mock-test DB
    const MockResult = await getMockResultModel();
    await Promise.all([
      Result.deleteMany({ user_id: Number(id) }),
      MockResult.deleteMany({ user_id: Number(id) }),
    ]);
...
export async function getDashboard(req, res, next) {
  try {
    const data = await getOrSetCached('admin:dashboard:v1', 800, async () => {
      const [studentsCount, testsCount, videosCount, materialsCount] = await Promise.all([
        User.countDocuments({ role: 'student' }).catch(() => 0),
        (await getMockTestModel()).countDocuments({ is_active: true }).catch(() => 0),
        Video.countDocuments({ status: 'active' }).catch(() => 0),
        (await getMaterialModel()).countDocuments({}).catch(() => 0),
      ]);
...
export async function createMaterial(req, res, next) {
  try {
    ...
    const m = new (await getMaterialModel())({
      title,
      pdf_url: pdfUrl,
      subject,
      type,
...
export async function listMaterials(req, res, next) {
  try {
    ...
    const rows = await (await getMaterialModel()).find(filter, { _id: 0, __v: 0 })
      .sort({ created_at: -1, id: -1 })
      .lean();
...
export async function updateMaterial(req, res, next) {
  try {
    ...
    if (Object.keys(updates).length > 0) {
      if (updates.pdf_url) {
        const existing = await (await getMaterialModel()).findOne({ id: Number(id) }, { pdf_url: 1 }).lean();
        if (existing && existing.pdf_url && existing.pdf_url !== updates.pdf_url) {
          await deleteFromGridFSByUrl(existing.pdf_url);
        }
      }
      updates.updated_at = new Date();
      await (await getMaterialModel()).updateOne({ id: Number(id) }, { $set: updates });
    }

    const m = await (await getMaterialModel()).findOne({ id: Number(id) }, { _id: 0, __v: 0 }).lean();
...
export async function deleteMaterial(req, res, next) {
  try {
    ...
    const existing = await (await getMaterialModel()).findOne({ id: Number(id) }, { id: 1, pdf_url: 1, _id: 0 }).lean();
    ...
    await (await getMaterialModel()).deleteOne({ id: Number(id) });
    return res.json({ ok: true });
  } catch (err) {
    return next(err);
  }
...
export async function createTestWithQuestions(req, res, next) {
  try {
    ...
    const normalizedQuestions = questions.map((q, idx) => {
      ...
      // Image is completely optional now
...
export async function listTests(req, res, next) {
  try {
    const MockTest = await getMockTestModel();
    const rows = await MockTest.find({}, { _id: 0, __v: 0 }).sort({ created_at: -1, id: -1 }).lean();
...
export async function updateTest(req, res, next) {
  try {
    ...
    if (Object.keys(updates).length > 0) {
      updates.updated_at = new Date();
      await MockTest.updateOne({ id: Number(id) }, { $set: updates });
    }

    const t = await MockTest.findOne({ id: Number(id) }, { _id: 0, __v: 0 }).lean();
...
export async function deleteTest(req, res, next) {
  try {
    ...
    let existing = await MockTest.findOne({ id: Number(id) }, { id: 1, _id: 0 }).lean();
    if (!existing) return res.status(404).json({ message: 'Test not found' });

    await MockResult.deleteMany({ test_id: Number(id) });
    await MockQuestion.deleteMany({ test_id: Number(id) });
    await MockTest.deleteOne({ id: Number(id) });
...
export async function listResults(req, res, next) {
  try {
    ...
    if (testId) {
      ...
      // No $limit here — return ALL students who attended this test
      const firstAttempts = await MockResult.aggregate([
        { $match: { test_id: tid } },
        { $sort: { id: 1 } },
      ...
      const [users, test] = await Promise.all([
        User.find({ id: { $in: userIds } }, { _id: 0, id: 1, name: 1, email: 1 }).lean(),
        MockTest.findOne({ id: tid }, { _id: 0, id: 1, title: 1 }).lean(),
      ]);

      const userById = new Map((users || []).map((u) => [Number(u.id), u]));
      const out = firstAttempts.map((r, idx) => {
        const u = userById.get(Number(r.user_id));
        return { ...r, student_name: u?.name ?? null, student_email: u?.email ?? null, test_title: test?.title ?? null, rank: idx + 1 };
      });

      return res.json({ results: out, total: out.length });
    }

    // No testId — recent results overview, capped at 500
    const rows = await MockResult.find({}, { _id: 0, __v: 0 }).sort({ date: -1, id: -1 }).limit(500).lean();
    ...
    const [users, tests] = await Promise.all([
      User.find({ id: { $in: userIds } }, { _id: 0, id: 1, name: 1, email: 1 }).lean(),
      MockTest.find({ id: { $in: testIds } }, { _id: 0, id: 1, title: 1 }).lean(),
    ]);
    ...
    const out = rows.map((r) => {
      ...
      return { ...r, student_name: u?.name ?? null, student_email: u?.email ?? null, test_title: t?.title ?? null };
    });
  ...
  try {
    ...
    if (testId) {
      ...
      rows = firstAttempts.map((r) => {
        const u = userById.get(Number(r.user_id));
        return { date: r.date, student_name: u?.name ?? null, student_email: u?.email ?? null, test_title: test?.title ?? null, score: r.score, accuracy: r.accuracy, time_taken_sec: r.time_taken_sec };
      });
    } else {
      const results = await MockResult.find({}, { _id: 0, date: 1, user_id: 1, test_id: 1, score: 1, accuracy: 1, time_taken_sec: 1 }).sort({ date: -1, id: -1 }).limit(5000).lean();
      ...
      const userById = new Map((users || []).map((u) => [Number(u.id), u]));
      const testById = new Map((tests || []).map((t) => [Number(t.id), t.title]));
      rows = results.map((r) => {
        const u = userById.get(Number(r.user_id));
        const testTitle = testById.get(Number(r.test_id));
        return { date: r.date, student_name: u?.name ?? null, student_email: u?.email ?? null, test_title: testTitle ?? null, score: r.score, accuracy: r.accuracy, time_taken_sec: r.time_taken_sec };
      });
    }
...
export async function listPyqs(req, res, next) {
  try {
    const PyqModel = await getPyqModel();
    const rows = await PyqModel.find({}, { _id: 0, __v: 0 })
      .sort({ created_at: -1, id: -1 })
      .lean();
    ...
    const out = pyqList.map((p) => ({
      ...p,
      centre_name: p.centre_id ? (centreById.get(Number(p.centre_id))?.name ?? null) : null,
    }));

    return res.json({ pyqs: out });
  } catch (err) {
...
export async function updatePyq(req, res) {
  try {
    ...
    const PyqModel = await getPyqModel();
    const existing = await PyqModel.findOne({ id: Number(id) }, { _id: 0, id: 1, centre_id: 1, year: 1 }).lean();
    ...
    if (centre_id !== undefined) {
      if (centre_id === '' || centre_id === null) {
        ...
        const ExamCentreModel = await getExamCentreModel();
        const centre = await ExamCentreModel.findOne({ id: Number(parsed) }, { id: 1, _id: 0 }).lean();
        if (!centre) return res.status(400).json({ message: 'Invalid centre_id' });
        nextCentreId = parsed;
      }
    }
    ...
    if (updates.pdf_url !== undefined || updates.solution_url !== undefined) {
      const existingPyq = await PyqModel.findOne({ id: Number(id) }, { pdf_url: 1, solution_url: 1 }).lean();
      if (existingPyq) {
        if (updates.pdf_url && existingPyq.pdf_url && existingPyq.pdf_url !== updates.pdf_url) {
          await deleteFromGridFSByUrl(existingPyq.pdf_url);
        }
        if (updates.solution_url && existingPyq.solution_url && existingPyq.solution_url !== updates.solution_url) {
          await deleteFromGridFSByUrl(existingPyq.solution_url);
        }
      }
    }

    updates.updated_at = new Date();
    await PyqModel.updateOne({ id: Number(id) }, { $set: updates });
...
export async function getAnalytics(req, res) {
  try {
    const data = await getOrSetCached('admin:analytics:v1', 800, async () => {
      const [
        studentsCount,
        testsCount,
        specimensCount,
        ...
        (await getMockTestModel()).countDocuments({ is_active: true }),
        Specimen.countDocuments({ status: 'active' }),
        Video.countDocuments({ status: 'active' }),
        (await getMaterialModel()).countDocuments({}),
        (await getPyqModel()).countDocuments({}),
        (await getMockResultModel()).countDocuments({}),
        (await getMockResultModel()).aggregate([{ $group: { _id: null, avg: { $avg: '$accuracy' } } }]).catch(() => []),
      ]);
...
export async function addTestQuestion(req, res, next) {
  try {
    const { id } = req.params;
    const body = req.body || {};
    const MockTest = await getMockTestModel();
    ...
    const newQuestionRow = new MockQuestion({
      test_id: Number(id),
      question_text: body.questionText || 'New Question',
      image_url: body.imageUrl || '',
      option_a: body.optionA || 'Option A',
      option_b: body.optionB || 'Option B',
      option_c: body.optionC || 'Option C',
      option_d: body.optionD || 'Option D',
...
export async function updateTestQuestion(req, res, next) {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'questionText')) updates.question_text = body.questionText;
    if (Object.prototype.hasOwnProperty.call(body, 'imageUrl')) updates.image_url = body.imageUrl;
    if (Object.prototype.hasOwnProperty.call(body, 'optionA')) updates.option_a = body.optionA;
    if (Object.prototype.hasOwnProperty.call(body, 'optionB')) updates.option_b = body.optionB;
    if (Object.prototype.hasOwnProperty.call(body, 'optionC')) updates.option_c = body.optionC;
    if (Object.prototype.hasOwnProperty.call(body, 'optionD')) updates.option_d = body.optionD;
    if (Object.prototype.hasOwnProperty.call(body, 'correctOption')) updates.correct_option = body.correctOption;
...
export async function deleteTestQuestion(req, res, next) {
  try {
    const { id, questionId } = req.params;
    const MockTest = await getMockTestModel();
    const MockQuestion = await getMockTestQuestionModel();
    ...
    // Delete from new DB
    const r = await MockQuestion.deleteOne({ id: Number(questionId), test_id: Number(id) });
    if (r.deletedCount > 0) {
      await MockTest.updateOne({ id: Number(id) }, { $inc: { question_count: -1 } });
    }
