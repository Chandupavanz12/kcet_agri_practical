  ...
  new Schema(
    {
      ...
      // Set by admin when they manually revoke access. Used by the auto-recovery
      // payment check so it does NOT re-grant access after an intentional revocation
      // (unless the student pays again AFTER the revocation timestamp).
      admin_revoked_at: { type: Date, default: null },
      created_at: { type: Date, default: () => new Date() },
      updated_at: { type: Date, default: () => new Date() },
    },
...
export async function getMaterialModel() {
  const dbModule = await import('../config/db.js');
  const conn = dbModule.getFilesConnection();
  if (conn && conn !== mongoose.connection) {
    return conn.models.Material || conn.model('Material', MaterialSchema);
  }
  return Material;
}
...
export async function getExamCentreModel() {
  const dbModule = await import('../config/db.js');
  const conn = dbModule.getFilesConnection();
  if (conn && conn !== mongoose.connection) {
    return conn.models.ExamCentre || conn.model('ExamCentre', ExamCentreSchema);
  }
  return ExamCentre;
}
...
export async function getExamCentreYearModel() {
  const dbModule = await import('../config/db.js');
  const conn = dbModule.getFilesConnection();
  if (conn && conn !== mongoose.connection) {
    return conn.models.ExamCentreYear || conn.model('ExamCentreYear', ExamCentreYearSchema);
  }
  return ExamCentreYear;
}
...
export async function getPyqModel() {
  const dbModule = await import('../config/db.js');
  const conn = dbModule.getFilesConnection();
  if (conn && conn !== mongoose.connection) {
    return conn.models.Pyq || conn.model('Pyq', PyqSchema);
  }
  return Pyq;
}
...
const RegisterOtpSchema = withNumericId(
  new Schema(
    {
...
RegisterOtpSchema.index({ email: 1, token: 1 }, { unique: true });
const RegisterOtp = mongoose.models.RegisterOtp || mongoose.model('RegisterOtp', RegisterOtpSchema);
...
export {
  Counter, ExamCentre,
  ExamCentreYear, Feedback, LoginOtp, Material, MaterialCompletion, Menu, Notification, PasswordReset, Payment, Plan, Pyq, RegisterOtp, Result, Settings, Specimen,
  Test,
  TestQuestion, User, UserAccess, UserNotification, Video
};
